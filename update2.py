...
notification.notify(title = '發行圖面通知系統', message = '開始啟動偵測中' ,app_icon ='C:/Users/cnc-3/Downloads/light.ico', timeout = 2 )#--公司用
    ...
    def checkSnapshot(self):
        ...
        if diff.files_created == []:
            ...
            for dc in diff.files_created:
                ...
                notification.notify(title = '發行圖-新增', message = dc[14:] ,app_icon ='C:/Users/cnc-3/Downloads/light.ico', timeout = 0.3 )
                
                #data = {'message':ans}     # 設定要發送的訊息
                #data = requests.post(url, headers=headers, data=data)   # 使用 POST 方法
                if os.path.isfile('C:/Users/Public/Documents/發行圖面更新紀錄.csv'): #檢查檔案在不在
        ...
        for dd in diff.files_deleted:
            ...
            notification.notify(title = '發行圖-刪除', message = dd[14:] ,app_icon ='C:/Users/cnc-3/Downloads/light.ico', timeout = 0.3 )
        ...
        if len(dm_ans) < 1:
            ...
            print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())), Fore.BLUE +'圖面變更資訊: 檔案從', dm_ans[0][0],'  變更為 >>> ', Fore.CYAN + dm_ans[0][1])
            notification.notify(title = '發行圖-變更資訊', message = '圖面有變更資訊' ,app_icon ='C:/Users/cnc-3/Downloads/light.ico', timeout = 0.3 )
            if os.path.isfile('C:/Users/Public/Documents/發行圖面更新紀錄.csv'): #檢查檔案在不在
                with open('C:/Users/Public/Documents/發行圖面更新紀錄.csv', 'r', encoding = 'cp950') as f:
                    for txtlog in f:
                        log.append(str(txtlog))
                with open('C:/Users/Public/Documents/發行圖面更新紀錄.csv', 'w', encoding = 'cp950') as f:
                    for l in log:
                        f.write(str(l))
                    f.write(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))) + '圖面變更資訊: 檔案從' + str(dm_ans[0][0]) + '  變更為 >>> ' + str(dm_ans[0][1]) + '\n')
            else:
                with open('C:/Users/Public/Documents/發行圖面更新紀錄.csv', 'w', encoding = 'cp950') as f:
                    for l in log:
                        f.write(str(l))
        ...
        if len(dmf_ans) < 1:
            ...
            notification.notify(title = '發行圖-檔案修改', message = str(dmf_ans[0]) + '內容已修改' ,app_icon ='C:/Users/cnc-3/Downloads/light.ico', timeout = 0.3 )
            if os.path.isfile('C:/Users/Public/Documents/發行圖面更新紀錄.csv'): #檢查檔案在不在
                with open('C:/Users/Public/Documents/發行圖面更新紀錄.csv', 'r', encoding = 'cp950') as f:
                    for txtlog in f:
                        log.append(str(txtlog))
